...
export default function FocusTimelineView({
  tasks,
  onUpdateStatus,
  onUpdateTask,
  onOpenEdit,
  ...
  // Recommended Focus Spotlight Calculation
  const activeUnfinished = eligibleTasks.filter((t) => t.status !== 'done');
  
  const focusRecommendations = [...activeUnfinished].sort((a, b) => {
    // Score calculation
    const getScore = (t) => {
      let score = 0;
      const isOverdue = t.due && t.due < todayStr;
      const isToday = t.due && t.due === todayStr;
      const isHigh = t.priority === 'high';
      const isDoing = t.status === 'doing';

      if (isOverdue && isHigh) score += 1000;
      else if (isOverdue) score += 500;
      else if (isToday && isHigh) score += 400;
      else if (isToday) score += 300;
      else if (isHigh) score += 200;

      if (isDoing) score += 150;
      return score;
    };
  ...
  return (
    <div className="space-y-6 animate-fadeIn text-slate-100 font-sans pb-12">
      ...
      {/* 🎯 SMART FOCUS SPOTLIGHT SECTION */}
      {focusRecommendations.length > 0 && (
        <section className="bg-gradient-to-r from-[#1e1f20] via-[#1a1c23] to-[#1e1f20] border border-[#8ab4f8]/30 rounded-3xl p-5 shadow-xl relative overflow-hidden">
          <div className="absolute top-0 right-0 w-64 h-64 bg-[#8ab4f8]/5 rounded-full blur-3xl pointer-events-none"></div>
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <h3 className="text-sm font-bold text-slate-100 flex items-center gap-2">
                  ...
                  <span className="text-[10px] bg-[#8ab4f8] text-[#0f172a] font-bold px-2 py-0.5 rounded-full">
                    Top Priority
                  </span>
                </h3>
                ...
                <div
                  ...
                  className={`p-4 rounded-2xl border transition-all flex flex-col justify-between ${
                    isOverdue && isHighPriority
                      ? 'bg-[#f28b82]/10 border-[#f28b82]/50 shadow-lg shadow-[#f28b82]/10'
                      : isOverdue
                      ? 'bg-[#f28b82]/5 border-[#f28b82]/30'
                      : isHighPriority
                      ? 'bg-[#fdd663]/10 border-[#fdd663]/40'
                      : 'bg-[#131314] border-[#3c4043]'
                  }`}
                >
                  <div>
                    {/* Reason Badges */}
                    <div className="flex flex-wrap items-center gap-1.5 mb-2">
                      {isOverdue && isHighPriority && (
                        <span className="bg-[#f28b82] text-[#0f172a] font-bold text-[10px] px-2 py-0.5 rounded-full flex items-center gap-1">
                          <Flame className="w-3 h-3 fill-[#0f172a]" /> CRITICAL OVERDUE
                        </span>
                      )}
                      {isOverdue && !isHighPriority && (
                        <span className="bg-[#f28b82]/20 text-[#f28b82] border border-[#f28b82]/40 text-[10px] font-bold px-2 py-0.5 rounded-full flex items-center gap-1">
                          <AlertTriangle className="w-3 h-3" /> Overdue
                        </span>
                      )}
                      {isHighPriority && !isOverdue && (
                        <span className="bg-[#fdd663]/20 text-[#fdd663] border border-[#fdd663]/40 text-[10px] font-bold px-2 py-0.5 rounded-full flex items-center gap-1">
                          <Flame className="w-3 h-3" /> High Priority
                        </span>
                      )}
                      {task.status === 'doing' && (
                        <span className="bg-[#8ab4f8]/20 text-[#8ab4f8] border border-[#8ab4f8]/40 text-[10px] font-bold px-2 py-0.5 rounded-full">
                          In Progress
                        </span>
                      )}
                    </div>
                    ...
                    <div className="flex items-center gap-2 text-[11px] text-slate-400 mb-3">
                      ...
                      {task.due && (
                        <span className={`font-semibold ${isOverdue ? 'text-[#f28b82]' : 'text-slate-300'}`}>
                  ...
                  {/* Actions */}
                  <div className="flex items-center justify-between pt-2 border-t border-[#3c4043]/50">
                    <button
                      onClick={() => onUpdateStatus(task.id, 'doing')}
                      title="Focus Now: Sets task status to 'doing' (In Progress) in the vault and marks it as your active focus task."
                      className="px-2.5 py-1 bg-[#8ab4f8] hover:bg-[#a8c7fa] text-[#0f172a] text-[11px] font-bold rounded-lg flex items-center gap-1 transition-all cursor-pointer shadow-sm"
                    >
                      <Play className="w-3 h-3 fill-[#0f172a]" /> Focus Now
                    </button>
                    <button
                      onClick={() => onUpdateStatus(task.id, 'done')}
                      title="Mark task as complete ('done')"
                      className="px-2.5 py-1 bg-[#81c995]/20 hover:bg-[#81c995]/30 text-[#81c995] border border-[#81c995]/40 text-[11px] font-bold rounded-lg flex items-center gap-1 transition-all cursor-pointer"
                    >
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between border-b border-[#3c4043] pb-2">
          ...
          <span className="text-xs text-slate-400">Use quick scheduler to set focus dates</span>
        </div>
          ...
          <div className="grid grid-cols-1 gap-2.5">
            ...
            {unscheduledTasks.length > 15 && (
              <div className="p-3 text-center text-xs text-slate-400 bg-[#131314] rounded-2xl border border-[#3c4043]">
                + {unscheduledTasks.length - 15} more unscheduled items in backlog
              </div>
            )}
          </div>
        )}
